...
def zoom_out(img,zoom,random_value,b):
    a=img.copy()
    x,y,w,h=b
    w=w-x
    h=h-y
...
out.write('name,iw,ih,x1,y1,x2,y2,class\n')
...
for i in f.iterrows():
	...
	b=[x1,y1,x2,y2]
	#zoom=30
	translate,sheared,angle,zoom=20,20,60,20
	img3,b=zoom_out(img,zoom,rv,b)
	img3,b=up_to_down(img3,rv1,b)
	#display(img3,b)
	...
	cd=b
	ih,iw,ic=img3.shape
	if b[0]<0 or b[1]<0 or b[2]<0 or b[3]<0 or b[2]>iw or b[3]>ih:
		continue
	if name1!=name:
		name='./final_images/'+name.split('/')[-1]
		cv2.imwrite(name,img3)
	name='./final_images/'+name.split('/')[-1]
	out.write('{},{},{},{},{},{},{},{}\n'.format(name,iw,ih,cd[0],cd[1],cd[2],cd[3],cl))
	#break
